...
export class Utilities {
  ...
  public static printDuration(start: Date, end: Date) {
    ...
    if (days) {
      printedDays = `${days} дней`;
    }

    if (hours) {
      printedDays += printedDays ? `, ${hours} часов` : `${hours} часов`;
    }

    if (minutes) {
      printedDays += printedDays ? `, ${minutes} минут` : `${minutes} минут`;
    }

    if (seconds) {
      printedDays += printedDays ? ` и ${seconds} секунд` : `${seconds} секунд`;
    }
  ...
  public static isOvertime(overTime: number, lastUpdate: string, isActive: boolean): boolean {
    /*const currentDate =  Date.now();
    console.log('compareTime currentDate = ' + currentDate);
    const deviceTime = new Date(deviceDate).getTime();
    console.log('compareTime deviceTime = ' + deviceTime);*/
    const differTimeInMin =  this.getDifferentTime(lastUpdate);  //  Math.floor((currentDate - deviceTime) / 60000);
    /*console.log('**** isOverTime this.overTime = ' + overTime);
    console.log('compareTime differTime = ' + differTimeInMin);*/
    return isActive ? differTimeInMin > overTime : false;
  }

  public static getDifferentTime(deviceDate: string): number {
    const currentDate =  Date.now();
    // console.log('compareTime currentDate = ' + currentDate);
    const deviceTime = new Date(deviceDate).getTime();
    // console.log('compareTime deviceTime = ' + deviceTime);
    return  Math.floor((currentDate - deviceTime) / 60000);
  }
